...
import { View, Text, StyleSheet,StatusBar,Image,ScrollView,FlatList,TouchableOpacity } from 'react-native';
...
const RestaurantScreen = ({navigation, route:{params :{restaurantId},},}) => {
  ...
  return (
    <View style={styles.container}>
      <StatusBar barStyle={'default'} translucent backgroundColor='transprarent' />
      
        <Image source={{uri:StaticImageService.getGalleryImage(restaurant?.images?.cover,
          ...
          <View style={styles.mainContainer}>
            ...
            <View style={styles.categoriesContainer}>
              <FlatList
                ...
                renderItem={({item}) => 
                    <CategoryListItem  name={item} isActive={item === selectedCategory} selectCategory={category => setSelectedCategory(category)}/>}
              />
            </View>
            <View style={styles.foodList}>
                {restaurant?.foods?.filter(food => food?.category === selectedCategory)?.map( item => ( <FoodCard key={item?.id} {...item}/> ))}
            </View>
          </View>
    </ScrollView>
